...
Also check `catalog/searches/dedup_registry.json` for previously searched term × field combinations and known YEK IDs. This is a separate structured index from MEMORY.md — both must be consulted. After completing a search, update the dedup registry with new combinations and any new YEK IDs encountered.
...
- After completing a search, return results to the orchestrator. Recommend next steps (e.g., `motif-classifier` for image analysis, `metadata-generator` for structured cataloguing) but do not invoke other agents directly.
